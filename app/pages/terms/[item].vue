...
setI18nParams({
  en: { item: data[0].slug.find(t => t._key === 'en').value.current.split('/').filter(Boolean).pop() },
  es: { item: data[0].slug.find(t => t._key === 'es').value.current.split('/').filter(Boolean).pop() }
})
