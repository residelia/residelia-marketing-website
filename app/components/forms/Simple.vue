...
// console.log(locale)
// console.log(formData[0].error.find(t => t._key === locale.value).value)
...
const rules = computed(() => {
  return {
    email: {
      required: helpers.withMessage(formData[0].inputs.find(x => x.name === 'email').validation.find(x => x.type === 'required').message.find(t => t._key === locale.value).value, required),
      email: helpers.withMessage(formData[0].inputs.find(x => x.name === 'email').validation.find(x => x.type === 'email').message.find(t => t._key === locale.value).value, email),
      $autoDirty: true,
    },
  };
});
...
async function onSubmit(e) {
    ...
    if (!v$.value.$error) {
      ...
      if (formResult.success) {
        formResult.message = formData[0].success.find(t => t._key === locale.value).value
      } else {
        formResult.message = formData[0].error.find(t => t._key === locale.value).value
      }
      clearForm()
      v$.value.$reset()
    } else {
