                    ...
                    <div class="hero-17-txt">
                        ...
                        <div v-if="hero.buttons" class="d-grid gap-2 d-md-block mx-auto">
                            <template v-for="(button,index) in hero?.buttons" >
                                <NuxtLink type="button" v-if="button.button.linkType === 'anchor'" :to="button.button.anchor" class="btn r-04 mr-5 hover--theme" :class="{'btn--theme': index === 0, 'btn--theme-secondary': index !== 0}" @click="handleClick(button.button.id,'home-hero-button')" v-follow >{{ button.button.linkText.filter(l => l._key === locale.slice(0,2))[0]?.value }}</NuxtLink>
                                <a type="button"  v-else-if="button.button.linkType === 'external'" :href="button.button.externalUrl" class="btn r-04 mr-5 hover--theme" :class="{'btn--theme': index === 0, 'btn--theme-secondary': index !== 0}" @click="handleClick(button.button.id,'home-hero-button')" target='_blank' external v-follow>{{ button.button.linkText.filter(l => l._key === locale.slice(0,2))[0]?.value }}</a>
                                <NuxtLink type="button"  v-else-if="button.button.linkType === 'internal'" :to="$localePath(button.button.link.slug.find(l => l._key === locale).value.current+(button.button.queryString ? button.button.queryString : ''))" class="btn r-04 mr-5 hover--theme" :class="{'btn--theme': index === 0, 'btn--theme-secondary': index !== 0}" @click="handleClick(button.button.id,'home-hero-button')" v-follow >{{ button.button.linkText.filter(l => l._key === locale.slice(0,2))[0]?.value }}</NuxtLink>
