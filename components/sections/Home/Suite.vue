<template>
    <!-- DIVIDER LINE -->
    <hr class="divider" />
    <section v-if="suite" id="features-2" class="pt-100 features-section division">
        <div class="container">
            <!-- SECTION TITLE -->
            <div class="row justify-content-center">
                <div class="col-md-10 col-lg-9">
                    <div class="section-title mb-70">
                        ...
                        <!-- Title -->
                        <h2 class="s-50 w-700">{{ suite.heading.find(t => t._key === locale).value}}</h2>
                        <!-- Text -->
                        <p class="s-21 color--grey">{{ suite.subHeading.find(t => t._key === locale).value }}</p>
                    </div>
                </div>
            </div>
            <!-- 1x3 WRAPPER -->
            <div v-if="suite.suite.length > 2" class="fbox-wrapper text-center mb-100">
                <div class="row row-cols-1 row-cols-md-3">
                    ...
                    <div v-for="product,index in suite.suite" class="col mb-50">
                        <div class="fbox-2 wow fadeInUp" :class="`fb-${index+1}`">
                            ...
                            <!-- Text -->
                            <div class="fbox-txt">
                                <h6 class="s-22 w-700">{{ product.heading.find(t => t._key === locale).value }}</h6>
                                <p>{{ product.subHeading.find(t => t._key === locale).value }}</p>
                            </div>
                        </div>
                    </div>
                    <!-- END FEATURE BOX -->
            ...
            <!-- END 1x3 WRAPPER -->
            <!-- 1x2 WRAPPER -->
            <div v-else class="fbox-wrapper text-center">
