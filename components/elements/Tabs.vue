    ...
    setup(props: IProps, { emit, slots, attrs }) {
        ...
        return () => {
            ...
            _tabItems.value.forEach((tab: VNode, index: number) => {
                ...
                const titleContent = _tabProps["title-slot"] ? getTitleSlotContent(_tabProps["title-slot"]) : _tabProps.title;
                ...
                tabList.push(
                    h(
                        "div",
                        {
                            class: selectedIndex.value === index ? "tab-link tab-list__item current" : "tab-link tab-list__item",
                            tabIndex: "0",
                            role: "tabItem",
            ...
            return h(
                ...
                [h("div", { class: `tab-list ${position.value}`, role: "tabList" }, tabList), ...tabToDisplay.value]
            );
        };
    }
});
